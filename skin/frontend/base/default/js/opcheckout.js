...
Checkout.prototype = {
    ...
    _colsePageTransform: function(){
        $j('.page').removeClass('open-step');
        $j('.page').scrollTop(0);
    },

    _updatePrice: function(obj){
        $j('.order-total .price').replaceWith(obj.grandtotal);
        $j('.grand-total .price').replaceWith(obj.grandtotal);
        $j('.express-prices .price').replaceWith(obj.shipping);
        $j('.tax-prices .price').replaceWith(obj.tax);
    },
            ...
            new Ajax.Request(
                this.saveMethodUrl,{
                    ...
                    onSuccess: function(res){
                        ...
                        $j('#opc-login .step-result').html('<span>'+email+'</span>');
                        $j('#billing\\:email')
                        .val(email)
                        .attr('type','hidden')
                    }
...
Billing.prototype = {
    ...
    nextStep: function(transport){
        ...
        var street = response.preinfo.street[0];
        var city =response.preinfo.city;
        $j('#opc-billing .step-result').html(
            '<span>'+street+'</span>'+
            '<span>'+city+'</span>'
        );
        if(response.preinfo.use_for_shipping === '1'){
            $j('#opc-shipping .step-result').html(
                '<span>'+street+'</span>'+
                '<span>'+city+'</span>'
            )
        }else{
        ...
        checkout._updatePrice(response.summary);
...
Shipping.prototype = {
    ...
    setSameAsBilling: function(flag) {
        $('shipping:same_as_billing').checked = flag;
        // Also it hangs up, if the flag is not false
...
ShippingMethod.prototype = {
    ...
    nextStep: function(transport){
        ...
        $j('#opc-shipping_method .step-result').html(response.preinfo);
        checkout._updatePrice(response.summary);
...
Payment.prototype = {
    ...
    nextStep: function(transport){
        ...
        $j('#opc-payment .step-result').html(response.preinfo);
        checkout._updatePrice(response.summary);
        checkout.setStepResponse(response);
