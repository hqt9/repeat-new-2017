...
class Mage_Checkout_CartController extends Mage_Core_Controller_Front_Action
{
    ...
    protected function _updateShoppingCart()
    {
        try {
            ...
            // if (is_array($cartData) || is_array($super_attribute)) {
                $filter = new Zend_Filter_LocalizedToNormalized(
                    array('locale' => Mage::app()->getLocale()->getLocaleCode())
                );
                foreach ($cartData as $index => $data) {
                    if (isset($data['qty'])) {
                        $_params[$index]['qty'] = $filter->filter(trim($data['qty']));
                        $_params[$index]['super_attribute'] = $super_attribute[$index];
                        // $_params[$index]['form_key'] = $params['form_key'];
                        $_params[$index]['id'] = $index;
                        $_params[$index]['related_product'] = '';
                        $_params[$index]['product'] = $aparams['product'][$index];
