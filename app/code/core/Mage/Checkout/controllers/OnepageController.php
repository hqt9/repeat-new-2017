...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
    ...
    public function saveMethodAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $grandtotal = Mage::helper('checkout')->formatPrice($totals["grand_total"]->getValue());
            $shipping = Mage::helper('checkout')->formatPrice(Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getShipping_amount());
            $discount = 0;
            $tax = 0;
            if($totals['discount']){
            ...
            $discount = Mage::helper('checkout')->formatPrice($discount);
            $tax = Mage::helper('checkout')->formatPrice($tax);
            $result['summary'] = ['discount' => $discount, 'shipping' => $shipping, 'tax' => $tax, 'grandtotal' => $grandtotal];
            $result['preinfo'] = $method;
    ...
    public function saveShippingMethodAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $_rates = Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getShippingRatesCollection();
            $result['preinfo'] = '';
            foreach ($_rates as $_rate):
                if($data == $_rate->getCode()){
                    $result['preinfo'] = '<span class="s_method_'.$_rate->getCode().'">'.Mage::helper('core')->escapeHtml($_rate->getMethodTitle()).Mage::helper('checkout')->formatPrice($_rate->getPrice()).'</span>';
                }
            endforeach;
    ...
    public function savePaymentAction()
    {
        ...
        $_pay = Mage::getSingleton('checkout/session')->getQuote()->getPayment();
        // var_dump($_pay->getMethod());
        // var_dump($_pay->getMethodInstance()->getTitle());
        $result['preinfo'] = '';
        if($data['method'] == $_pay->getMethod()){
            $result['preinfo'] = '<span>'.Mage::helper('core')->escapeHtml($_pay->getMethodInstance()->getTitle()).'</span>';
        }
        $totals = Mage::getSingleton('checkout/session')->getQuote()->getTotals();
        $grandtotal = Mage::helper('checkout')->formatPrice($totals["grand_total"]->getValue());
        $shipping = Mage::helper('checkout')->formatPrice(Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getShipping_amount());
